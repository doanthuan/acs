...
//    }
    ...
    {
        return $this->updateStatus($cid, $value, $col);
    }

    public function updateStatus($cid, $status, $col = 'status')
    {
        if(!is_array($cid))
        {
            $cid = array($cid);
        ...
        $keyName = $this->getKeyName();
        $affectedRows = static::whereIn($keyName, $cid)->update(array($col => $status));
        return $affectedRows;
    }
