...
class ModelList{
    ...
    public function paginate($pageSize = null)
    {
        ...
        {
            //$defaultPageSize = \Doanthuan\Ladmin\Helper::getSetting('display.default_page_size');
            $defaultPageSize = 20;
    ...
    public function countInDateRange($range, $dateCol = 'created_at')
    {
        $query = $this->getSelect();
        if($range == 'day')
        {
            $query->selectRaw('HOUR('.$dateCol.') as num_item, count(*) as count');
            $query->whereRaw('date('.$dateCol.') = date(CURRENT_DATE)');
            $query->groupBy(DB::raw('HOUR('.$dateCol.')'));
        }
        if($range == 'month')
        {
            $query->selectRaw('DAY('.$dateCol.') as num_item, count(*) as count');
        ...
        if($range == 'year')
        {
            $query->selectRaw('MONTH('.$dateCol.') as num_item, count(*) as count');
            $query->whereRaw('YEAR('.$dateCol.') = YEAR(CURRENT_DATE)');
